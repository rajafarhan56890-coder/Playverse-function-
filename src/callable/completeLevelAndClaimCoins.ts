 ...
 */
export const completeLevelAndClaimCoins = onCall<CompleteLevelData>(
  async (request) => {
    ...
    if (
      typeof game.totalLevels !== "number" ||
      typeof game.coinsPerLevel !== "number"
    ) {
      throw new HttpsError(
        "failed-precondition",
        "This game has no valid level configuration."
      );
    }
    ...
    if (level > game.totalLevels) {
      throw new HttpsError(
        "invalid-argument",
        `This game only has ${game.totalLevels} levels.`
      );
    }
    ...
    const rawCompletedLevels = progressSnap.exists
      ? progressSnap.data()?.completedLevels
      : [];

    const completedLevels: number[] = Array.isArray(rawCompletedLevels)
      ? rawCompletedLevels.filter(
